...
const SocialMedia = () => {
  ...
  const handleAddPost = () => {
    ...
    const post = {
      id: posts.length + 1,
      content: newPost,
      author: userRole === 'admin' ? 'Admin' : 'Employee',
      likes: 0,
      timestamp: 'Just now',
                          ...
                          >
                            ...
                            <div className="flex items-start">
                              ...
                              <div className="flex-1 flex">
                                ...
                                />
                                <button
                                  className="bg-blue-600 text-white px-3 py-2 rounded-r-lg hover:bg-blue-700 transition text-sm"
...
//   const [newComment, setNewComment] = useState('');
//   const [commentingOnPost, setCommentingOnPost] = useState(null);
//   const [userRole, setUserRole] = useState(user?.role?.toLowerCase() || 'employee');
//   const [selectedImage, setSelectedImage] = useState(null);
//   const [imagePreview, setImagePreview] = useState(null);
//   const [isLoading, setIsLoading] = useState({ posts: false, notes: true, issues: false });
...
//     setError(null);
//     try {
//       const response = await fetch(`http://localhost:8080/api/issues?userId=${userId}`, {
//         headers: { "Authorization": `Bearer ${token}` }
//       });
...
//         },
//         body: JSON.stringify({
//           content: newNote   // only send content
//         })
//       });
...
//   const filteredNotes = notes.filter(note => {
//     if (activeNoteFilter === 'all') return true;
//     if (activeNoteFilter === 'admin') return note.userRole === 'ADMIN';
//     if (activeNoteFilter === 'employee') return note.userRole === 'EMPLOYEE';
//     return true;
//   });
...
//                       className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-4 py-2 rounded-lg hover:from-blue-700 hover:to-indigo-700 transition w-full flex items-center justify-center"
//                       onClick={handleAddPost}
//                       disabled={newPost.trim() === '' && !selectedImage || isLoading.posts}
//                     >
//                       {isLoading.posts ? (
//                         'Posting...'
//                       ) : (
...
//                       } text-white px-4 py-2 rounded-lg transition w-full flex items-center justify-center`}
//                     onClick={handleAddNote}
//                     disabled={newNote.trim() === '' || isLoading.notes}
//                   >
//                     {isLoading.notes ? (
//                       'Posting...'
//                     ) : (
...
//                 {isLoading.notes ? (
//                   <div className="space-y-4">
//                     <div className="bg-white rounded-xl shadow-md p-6 border-l-4 border-blue-500 bg-blue-50 animate-pulse h-32"></div>
//                     <div className="bg-white rounded-xl shadow-md p-6 border-l-4 border-green-500 bg-green-50 animate-pulse h-32"></div>
//                   </div>
//                 ) : filteredNotes.length === 0 ? (
//                   <motion.div
//                     className="bg-white rounded-xl shadow-md p-6 text-center"
...
//                       <motion.div
//                         key={note.id}
//                         className={`bg-white rounded-xl shadow-md p-6 border-l-4 ${note.authorRole === 'admin'
//                           ? 'border-blue-500 bg-blue-50'
//                           : 'border-green-500 bg-green-50'
//                           }`}
//                         initial={{ opacity: 0, y: 20 }}
...
//                         <div className="flex justify-between items-start mb-2">
//                           <div className="flex items-center">
//                             <div className={`rounded-full w-8 h-8 flex items-center justify-center font-semibold text-sm mr-3 ${note.authorRole === 'admin'
//                               ? 'bg-blue-100 text-blue-800'
//                               : 'bg-green-100 text-green-800'
//                               }`}>
...
//                             </div>
//                             <div>
//                               <h3 className="font-semibold">{note.authorName}</h3>
//                               <span className={`text-xs ${note.authorRole === 'admin'
//                                 ? 'text-blue-600'
//                                 : 'text-green-600'
//                                 }`}>
//                                 {note.authorRole === 'admin' ? 'Administrator' : 'Employee'}
//                               </span>
//                             </div>
//                           </div>
...
//                       className="bg-gradient-to-r from-red-600 to-pink-600 text-white px-4 py-2 rounded-lg hover:from-red-700 hover:to-pink-700 transition w-full flex items-center justify-center"
//                       onClick={handleAddIssue}
//                       disabled={newIssue.trim() === '' || isLoading.issues}
//                     >
//                       {isLoading.issues ? (
//                         'Submitting...'
//                       ) : (
...
//                   {isLoading.issues ? (
//                     <div className="space-y-4">
//                       <div className="bg-white rounded-xl shadow-md p-6 animate-pulse h-32"></div>
//                       <div className="bg-white rounded-xl shadow-md p-6 animate-pulse h-32"></div>
//                     </div>
//                   ) : issues.length === 0 ? (
//                     <motion.div
...
//                               <h3 className="font-semibold">{issue.author}</h3>
//                             </div>
//                             <span className="text-gray-500 text-sm">{new Date(issue.createdAt).toLocaleString()}</span>
//                           </div>
//                           <p className="mb-3 text-gray-700">{issue.content}</p>
//                           <div className="flex justify-between items-center">
//                             <span className={`text-sm px-3 py-1 rounded-full ${issue.status === 'open' ? 'bg-red-100 text-red-800' :
